// Code generated by protoc-gen-goext. DO NOT EDIT.

package agent

import (
	timestamppb "google.golang.org/protobuf/types/known/timestamppb"
)

type Test_Ammo = isTest_Ammo

func (m *Test) SetAmmo(v Test_Ammo) {
	m.Ammo = v
}

func (m *Test) SetId(v string) {
	m.Id = v
}

func (m *Test) SetFolderId(v string) {
	m.FolderId = v
}

func (m *Test) SetName(v string) {
	m.Name = v
}

func (m *Test) SetDescription(v string) {
	m.Description = v
}

func (m *Test) SetLabels(v map[string]string) {
	m.Labels = v
}

func (m *Test) SetCreatedAt(v *timestamppb.Timestamp) {
	m.CreatedAt = v
}

func (m *Test) SetStartedAt(v *timestamppb.Timestamp) {
	m.StartedAt = v
}

func (m *Test) SetFinishedAt(v *timestamppb.Timestamp) {
	m.FinishedAt = v
}

func (m *Test) SetUpdatedAt(v *timestamppb.Timestamp) {
	m.UpdatedAt = v
}

func (m *Test) SetGenerator(v Test_Generator) {
	m.Generator = v
}

func (m *Test) SetAgentInstanceId(v string) {
	m.AgentInstanceId = v
}

func (m *Test) SetTargetAddress(v string) {
	m.TargetAddress = v
}

func (m *Test) SetTargetPort(v int64) {
	m.TargetPort = v
}

func (m *Test) SetTargetVersion(v string) {
	m.TargetVersion = v
}

func (m *Test) SetConfig(v string) {
	m.Config = v
}

func (m *Test) SetAmmoUrls(v string) {
	m.Ammo = &Test_AmmoUrls{
		AmmoUrls: v,
	}
}

func (m *Test) SetAmmoId(v string) {
	m.Ammo = &Test_AmmoId{
		AmmoId: v,
	}
}

func (m *Test) SetCases(v []string) {
	m.Cases = v
}

func (m *Test) SetStatus(v Test_Status) {
	m.Status = v
}

func (m *Test) SetErrors(v []string) {
	m.Errors = v
}

func (m *Test) SetFavorite(v bool) {
	m.Favorite = v
}

func (m *LoadSchedule) SetType(v ScheduleType) {
	m.Type = v
}

func (m *LoadSchedule) SetInstances(v int64) {
	m.Instances = v
}

func (m *LoadSchedule) SetDuration(v string) {
	m.Duration = v
}

func (m *LoadSchedule) SetRpsFrom(v int64) {
	m.RpsFrom = v
}

func (m *LoadSchedule) SetRpsTo(v int64) {
	m.RpsTo = v
}

func (m *LoadSchedule) SetStep(v int64) {
	m.Step = v
}

func (m *LoadSchedule) SetOps(v int64) {
	m.Ops = v
}

func (m *LoadSchedule) SetStpdPath(v string) {
	m.StpdPath = v
}

func (m *Schedule) SetLoadType(v LoadType) {
	m.LoadType = v
}

func (m *Schedule) SetLoadSchedule(v []*LoadSchedule) {
	m.LoadSchedule = v
}

func (m *Schedule) SetLoadProfile(v []string) {
	m.LoadProfile = v
}

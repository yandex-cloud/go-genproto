...
const (
	...
	SubtitleService_GenerateDownloadURL_FullMethodName = "/yandex.cloud.video.v1.SubtitleService/GenerateDownloadURL"
...
type SubtitleServiceClient interface {
	...
	GenerateDownloadURL(ctx context.Context, in *GenerateSubtitleDownloadURLRequest, opts ...grpc.CallOption) (*GenerateSubtitleDownloadURLResponse, error)
...
func (c *subtitleServiceClient) GenerateDownloadURL(ctx context.Context, in *GenerateSubtitleDownloadURLRequest, opts ...grpc.CallOption) (*GenerateSubtitleDownloadURLResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(GenerateSubtitleDownloadURLResponse)
	err := c.cc.Invoke(ctx, SubtitleService_GenerateDownloadURL_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
type SubtitleServiceServer interface {
	...
	// This URL can be used to retrieve the subtitle content in its original format.
	// The URL is pre-signed and has a limited validity period.
	GenerateDownloadURL(context.Context, *GenerateSubtitleDownloadURLRequest) (*GenerateSubtitleDownloadURLResponse, error)
	// Generates a URL for uploading a subtitle file to an existing subtitle record.
	// This URL can be used to upload the actual subtitle file using an HTTP PUT request.
	// The URL is pre-signed and has a limited validity period.
	// (-- api-linter: yc::1705::http-method-mapping=disabled --)
...
func (UnimplementedSubtitleServiceServer) GenerateDownloadURL(context.Context, *GenerateSubtitleDownloadURLRequest) (*GenerateSubtitleDownloadURLResponse, error) {
...
func _SubtitleService_GenerateDownloadURL_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(GenerateSubtitleDownloadURLRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(SubtitleServiceServer).GenerateDownloadURL(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: SubtitleService_GenerateDownloadURL_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(SubtitleServiceServer).GenerateDownloadURL(ctx, req.(*GenerateSubtitleDownloadURLRequest))
	}
	return interceptor(ctx, in, info, handler)
}
...
var SubtitleService_ServiceDesc = grpc.ServiceDesc{
	...
	Methods: []grpc.MethodDesc{
		...
		},
		{
			...
			Handler:    _SubtitleService_GenerateDownloadURL_Handler,
		},
		{
